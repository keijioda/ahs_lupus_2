...
* To create an analytic file:
  ...
  * **Excluded those subjects who were diagnosed with SLE, but did not indicated whether they have been treated within 12 months or not (n = 76, see "Outcome" section below)**
...
* Subjects were classified into 3 groups of SLE diagnosis
  * Those who indicated "treated within the last 12 months" were classified into "Diagnosed & treated" regardless of whether diagnosis year is missing or not (n = 220).
  * Those who have ever diagnosed and indicated "not treated within the last 12 months" were classified into "Diagnosed but no treated" (n = 138).
  * Those who have never diagnosed were classified into "Not diagnosed" group.
  * Those who have ever diagnosed but have missing on "treated within the last 12 months" were excluded (n = 76).
...
* Descriptive table stratified by SLE diagnosis (not diagnosed vs treated + not treated together) 

```{r echo = FALSE}
table_vars <- c("age", "agecat", "black", "smkever", "alcever", "educat3", "vegstat3", "take_fo", "bmi", "bmicat", "kcal")
lupus %>% CreateTableOne(table_vars, strata = "sle_diag", addOverall = TRUE, data = .) %>%
  print(showAllLevels = TRUE, pDigits = 4) %>% 
  kableone()
```
...
* Median (IQR) intake (gram/day) of energy-adjusted ALA, EPA + DHA, total omega-3, and total omega-6 by SLE group:
...
* Four logistic models were fitted using SLE diagnosis (treated + not treated) as the outcome.
